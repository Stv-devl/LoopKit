...
A complete workflow for Claude Code: **one loop** from idea to commit, two
entry points depending on the size of the feature, 11 subagents, 10 declarative
rules and 14 hooks that mechanically refuse what the rules forbid.

Built for a React + TypeScript project, but the backend is a parameter: the core
assumes nothing about your database or your API.

```
                            /product          (once per product)
                                │
...
THE LOOP (identical for both)                         ▼
  /research ─▶ /interface ─▶ /plan ─▶ EXECUTE ─▶ /review ─▶ /ship ─▶ next feature
       └────────────── /orchestrate runs the whole thing ────────────┘
```
...
```bash
git clone <this-repo> loopkit
cd loopkit
./install.sh /path/to/my-project          # + --supabase and/or --fastapi
                                          # + --deploy=netlify|vercel|pages|ssh|ghcr|none
                                          # + --no-ci  (local project, no remote:
                                          #   installs no workflow at all)
                                          #   both asked interactively if omitted
```

The script copies `.claude/`, `CLAUDE.md` and the two `docs/` files
(`Claude_Workflows.md`, `ADAPTATION.md`) into the target project. It **never
overwrites** an existing file: it writes a `.new` alongside and lists the merges
left for you.

Then, one thing to do before running anything:

```bash
grep -rnE "FILL|CONFIGURE" .claude/ CLAUDE.md docs/
```

Every marker is a place where the kit is waiting for the reality of **your**
project — starting with the data client, which half the guardrails depend on. The
ordered checklist is in [`docs/ADAPTATION.md`](docs/ADAPTATION.md).

Then, inside the project:

```bash
/spec my feature                     # interview → docs/specs/<slug>.md
...
| ----------- | ------------------------------------------------------------------------------ | ------------------------------ |
| `/research` | Multi-modal fan-out: code, blast radius, **the real state of the database**, third-party APIs | `docs/work/<slug>/research.md` |
| `/interface` | Competing UI proposals (how many = the profile), rendered on an **editable canvas**, then **you decide**: human gate. SKIP when there is no UI | `docs/work/<slug>/design.md`   |
| `/plan`     | Write chain + parallelisable lots + contracts + test plan                      | `docs/work/<slug>/plan.md`     |
| EXECUTE     | Inline, sequential, in the plan's order                                        | the code                       |
| `/review`   | Visual pass (UI) · 5 dimensions in parallel · every finding refuted            | PASS/CONCERNS/FAIL verdict     |
| `/ship`     | Parallel gates, commit via the `github` agent, board updated, next feature named | the commit                     |

Every step is **callable on its own**, so you can rejoin mid-loop: after a manual
fix, `/review <artifact>` just replays the gate.

> **`/interface`, not `/design`.** Claude Code ships its own `/design` skill (the
...
> only its mirror: details in `docs/Claude_Workflows.md`.

### The product layer, above both entries

`/product` runs **once per product** and writes two files nothing else can
replace.
...
softening the story), and `/review`'s `correctness` dimension replays the same
invariants against the **code that was actually written**. Neither covers the
other, and a feature entering by `/spec` only ever meets the second.
...
against a diff, so on the light path `/spec` checks it once, out loud, at the
moment the contract is written.

So write both as sentences somebody who was not in the conversation can refuse
something with, not as intentions. `/product` reads them back and waits before it
...
diagram above. **One line per unit of loop**: a spec is one line, a full pipeline
is one line per story. Six states, one owner each: `/product` writes `DRAFT`
(noticed, framed by nothing) and it alone, which is what makes the gate able to
refuse; `/spec` and `/stories:review` write `READY` because a contract or a
reviewed slice exists behind the line (`/orchestrate` too, for the one case of an
artifact on disk and on no line); `/orchestrate` takes it to `IN LOOP`;
`/ship` closes it as `SHIPPED` and names the next `READY`. The two that keep the
...
written by the story gate, or by `/product` and `/ship` for a decision you take in
front of them, never inferred from a line looking old — and always with the story
file's `Status` moved in the same pass, since that is the half `/tracks` reads.

**The order of the rows is the priority** — the only place the kit writes one
down. `/ship` takes the *topmost* `READY`, `/product` is the only command that
...
The full state machine, its columns and its transitions live in `/product` and
...
**Simple** — one screen, a CRUD, a fix: `/spec` asks 3–4 questions, writes the
...
### The design system, a hard prerequisite of `/interface`

Without `docs/design-system.md`, interface proposals invent components instead of
composing with the ones that exist. `/design-system` produces that document, in
...
Full map: [`docs/Claude_Workflows.md`](docs/Claude_Workflows.md).
...
**How many at once is computed, not chosen.** `/tracks` reads the story map
...
security audit at `/ship` (see below).

**The profile is chosen at the product step, not at launch.** `/spec` and
`/bmad:pm` ask the closed question and write `Token profile:` at the top of the
spec or PRD, `/bmad:sm` copies it onto every story, and `/orchestrate` inherits it
...
```bash
/orchestrate --economy docs/stories/<slug>/1.1.md
...
`/review` has two stages. First `reviewer` agents whose default bias is "there is
...
| `security-auditor` | Security audit of **one surface**, on the whole repository (not on a diff) — out of the loop, except on a `critical` profile where `/ship` arms one | no |
...
cost paid unconditionally, before any work. Three exits for what does not fit,
...
| `eslint-batch`                    | end of turn / of agent | lints the whole queue in **one** ESLint process, non-blocking (the blocking lint stays `/ship`)   |
...
| --- | --- | --- |
| On the diff | `/review`'s `security` dimension — the server-side barrier, exposed keys, Zod at the boundaries, leaks between users | every review; a Critical means FAIL |
...
`/kit:doctor`'s `gate-allowlist` check watches this list against `/ship`'s gate
...
**Commands** (25) — loop: `/research` `/interface` `/plan` `/review` `/ship`
`/orchestrate` · entries: `/spec`, `/bmad:pm` `/bmad:sm` `/stories:review`
`/bmad:architect` `/bmad:dev` `/bmad:qa` `/bmad:flow` · framing: `/product`
`/design-system` · parallelism: `/tracks` · out of the loop:
`/database:migration` `/audit:security` `/audit:mutation` `/refactor:split`
`/refactor:clean` `/refactor:types` `/kit:doctor` · handover: `/handoff:codex`.
...
- A Node project with a package manager — the kit uses `pnpm` by default, to be
  replaced in `.claude/rules/00-project.md`, `/ship`'s gate block, the three
  hooks that shell out to it (`format-on-save`, `eslint-check`, `eslint-batch`)
  and `tdd-prove-red.sh` (`RUN_TESTS`).
